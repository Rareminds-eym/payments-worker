...
src/
  ...
  constants.ts          — All magic values: limits, timeouts, CORS fallback, error codes, SERVICE_ID
...
Managed in `src/utils/response.ts`. Allowed origins are configured via the `ALLOWED_ORIGINS` environment variable in `wrangler.toml` — no source code change needed to add or remove a domain.

**Format:** comma-separated string, set in `wrangler.toml [vars]`:
...
To add a new domain: edit `wrangler.toml` and redeploy. No CLI secrets command required, no source code change needed.

**Localhost origins** (`localhost:8788`, `localhost:5173`, `127.0.0.1:8788`, `127.0.0.1:5173`) are automatically appended when `ENVIRONMENT !== 'production'`. They are never active in production regardless of what `ALLOWED_ORIGINS` contains.

**Fallback:** if `ALLOWED_ORIGINS` is not set, no origins are allowed — CORS will be blocked for all requests. This is intentionally strict: misconfiguration is explicit rather than silently permissive.
...
| `ALLOWED_ORIGINS` | No | `wrangler.toml [vars]` / `.dev.vars` | Comma-separated CORS origin allowlist; falls back to `ALLOWED_ORIGINS_FALLBACK` if unset |
...
- CORS allowlist is driven by `ALLOWED_ORIGINS` env var — localhost origins blocked in production
